...
def brute_force_unique_groups(participants, teamHistory):
    # set up
    new_groups = np.zeros((math.floor(participants.size / MAX_TEAM_SIZE),MAX_TEAM_SIZE), dtype="object")
    ...
    while groupNum < (math.floor(participants.size / MAX_TEAM_SIZE)):
        ...
        for _ in range(1, MAX_TEAM_SIZE):
            potential_partners = tfh.optimalTeammateQueue(new_group, ungrouped, teamHistory)
            new_groupmate = pq.heappop(potential_partners)
            new_group.append(new_groupmate[1])
            ungrouped = np.delete(ungrouped, np.where(ungrouped==new_groupmate[1]))
    ...
    while ungrouped.size > 0:
        ...
        ungrouped = np.delete(ungrouped, np.where(ungrouped==leftover_person))
    ...
    temp_new_groups = np.array_split(flattened_groups, participants.size / MAX_TEAM_SIZE) # turn into mutable list
    ...
    for leftover in add_positions:
        ...
        temp_new_groups[leftover[1]] = np.append(temp_new_groups[leftover[1]],leftover[0])
        # print(temp_new_groups[leftover[1]])
